...
const state = {
  ...
  customers: [],
  documentCustomers: [],
  documentCustomersLoaded: false,
  documentCustomerRequestSequence: 0,
  documentModalOpenSequence: 0,
  products: [],
  productSuggestions: [],
...
async function loadInitialData() {
  const [me, settings, customers, products, productSuggestions] = await Promise.all([
    request('/auth/me'),
    request('/settings'),
    // Use `all` here deliberately.  Some older deployed backends still treat
    // a NULL `active` value as inactive when status=active is requested.
    // We apply the single source of truth (`isActiveRecord`) below instead.
    request('/customers?limit=200&page=1&status=all', { cache: 'no-store' }),
    request('/products?limit=200&page=1'),
    // Keep the page usable during a rolling deployment where the frontend
    // may become live a few seconds before the new backend route.
    request('/products/suggestions', { cache: 'no-store' }).catch(() => ({ data: [] }))
  ]);
  state.user = me.user;
  state.settings = settings.data;
  state.customers = customers.data;
  // The document modal owns its customer list. Do not let a slower initial
  // request overwrite a newer modal refresh when the user clicks Create early.
  if (!state.documentCustomersLoaded && state.documentCustomerRequestSequence === 0) {
    state.documentCustomers = normaliseDocumentCustomers(customers.data);
    state.documentCustomersLoaded = true;
  }
  state.products = products.data;
  state.productSuggestions = Array.isArray(productSuggestions.data) ? productSuggestions.data : products.data;
  applyRole();
  applyBrand();
...
function isActiveRecord(record) {
  ...
  // Older customer rows may not have an active value. Treat those rows as
  // active until a user explicitly deactivates them.
...
function normaliseDocumentCustomers(rows) {
  if (!Array.isArray(rows)) throw new Error('รูปแบบข้อมูลรายชื่อลูกค้าไม่ถูกต้อง');
  return rows
    .filter((customer) => customer && customer.id && String(customer.name || '').trim())
    // This list comes from status=all. Older database rows/views may omit
    // the active value, so a missing value must not hide a valid customer.
    .filter(isActiveRecord)
    .map((customer) => ({ ...customer, active: true }));
}

function setDocumentCustomerLoadStatus(message = '', { error = false, retry = false } = {}) {
  const status = $('#doc-customer-load-status');
  const retryButton = $('#doc-retry-customers');
  if (status) {
    status.textContent = message;
    status.classList.toggle('is-error', error);
  }
  if (retryButton) retryButton.classList.toggle('hidden', !retry);
}
...
function renderCustomerOptions() {
  ...
  if (!state.documentCustomersLoaded) {
    select.innerHTML = '<option value="">กำลังโหลดรายชื่อลูกค้า...</option>';
    return;
  }

  const activeCustomers = state.documentCustomers.filter(isActiveRecord);
  select.innerHTML = activeCustomers.length
    ? '<option value="">เลือกลูกค้า</option>'
      + activeCustomers.map((customer) => `<option value="${customer.id}">${escapeHtml(customer.name)} · ${CUSTOMER_TYPE_LABELS[customer.customer_type] || customer.customer_type || '-'}</option>`).join('')
    : '<option value="" disabled>ไม่พบลูกค้าที่เปิดใช้งาน</option>';

  if (selectedId && activeCustomers.some((customer) => String(customer.id) === String(selectedId))) {
...
async function refreshDocumentCustomers(selectedId = $('#doc-customer')?.value || '') {
  const select = $('#doc-customer');
  const requestSequence = ++state.documentCustomerRequestSequence;
  const cachedActiveCustomers = state.documentCustomers.filter(isActiveRecord);
  select.innerHTML = '<option value="">กำลังโหลดรายชื่อลูกค้า...</option>';
  select.disabled = true;
  setDocumentCustomerLoadStatus('กำลังโหลดรายชื่อลูกค้า...');
  try {
    // Document forms use their own unpaginated endpoint. This avoids coupling
    // the dropdown to the customer-list page/filter and handles legacy
    // active=NULL rows in the backend query itself.
    const result = await request(`/customers/document-options?_=${Date.now()}`, {
      cache: 'no-store',
      headers: { 'Cache-Control': 'no-cache' }
    });
    if (requestSequence !== state.documentCustomerRequestSequence) return false;
    state.documentCustomers = normaliseDocumentCustomers(result.data);
    state.documentCustomersLoaded = true;
  } catch (error) {
    if (requestSequence !== state.documentCustomerRequestSequence) return false;
    if (cachedActiveCustomers.length) {
      state.documentCustomers = cachedActiveCustomers;
      state.documentCustomersLoaded = true;
      setDocumentCustomerLoadStatus('โหลดรายชื่อใหม่ไม่สำเร็จ กำลังแสดงรายชื่อครั้งล่าสุด', { error: true, retry: true });
    } else {
      state.documentCustomers = [];
      state.documentCustomersLoaded = false;
      select.innerHTML = '<option value="" disabled>โหลดรายชื่อลูกค้าไม่สำเร็จ</option>';
      setDocumentCustomerLoadStatus('โหลดรายชื่อลูกค้าไม่สำเร็จ กรุณากดลองใหม่', { error: true, retry: true });
    }
    return false;
  } finally {
    if (requestSequence === state.documentCustomerRequestSequence) select.disabled = false;
  }
  if (requestSequence !== state.documentCustomerRequestSequence) return false;
  renderCustomerOptions();
  setDocumentCustomerLoadStatus('');
...
async function loadCustomers(search = '') {
  ...
  const result = await request(`/customers?limit=200&page=1&status=${status}&search=${encodeURIComponent(search)}`, { cache: 'no-store' });
  ...
  // Loading/searching the customer-management page must not be required to
  // populate (or accidentally replace) the document modal's customer list.
  bindMasterDataButtons(); refreshIcons();
}
...
function ensureProductDatalist() {
  ...
  // Legacy product rows may not have an `active` value.  As with customers,
  // hide only rows that were explicitly deactivated.
  const suggestions = state.productSuggestions.length ? state.productSuggestions : state.products;
  list.innerHTML = suggestions.filter((p) => p.active !== false).map((p) => {
    const source = p.source === 'document_history' ? 'จากเอกสารเดิม' : (p.sku || 'รายการเดิม');
    return `<option value="${escapeHtml(p.name)}">${escapeHtml(source)} · ${escapeHtml(p.unit || 'งาน')} · ${money(p.price)}</option>`;
  }).join('');
}

async function refreshProductSuggestions() {
  const result = await request('/products/suggestions', { cache: 'no-store' });
  state.productSuggestions = Array.isArray(result.data) ? result.data : [];
  ensureProductDatalist();
...
function renderDocumentActions(d) {
  ...
  if (caps.canEditMetadata) actions.push(actionIcon({ icon:'file-pen-line', title:'แก้ไขวันที่เอกสาร วันครบกำหนด หมายเหตุ หรือลายเซ็น', data:{ 'metadata-id':d.id }, className:'action-edit' }));
  ...
  if (!d.deleted_at && ['admin','staff'].includes(state.user?.role) && !['PAID','CANCELLED','REJECTED'].includes(d.status)) {
    if (d.document_type === 'QT') {
      ...
      if (['private','general'].includes(d.customer_type)) {
    ...
    if (d.document_type === 'DO' && ['private','general'].includes(d.customer_type)) {
      actions.push(actionIcon({ icon:'calendar-clock', title:'สร้างใบแจ้งหนี้จากเอกสารนี้', data:{ 'next-type':'IN', 'next-source':d.id }, className:'action-warning' }));
    }
    if (
...
const documentFlowCopy = {
  general: 'ใบเสนอราคา → ใบแจ้งหนี้ → รับชำระ หรือสร้างใบแจ้งหนี้โดยตรง',
...
const documentDetailCopy = {
  ...
  IN: ['จัดทำใบแจ้งหนี้', 'แจ้งยอดก่อนรับชำระ โดยเลือกใบเสนอราคาหรือกรอกรายการเองได้'],
...
async function selectDocumentType(type, { loadSources = true, applyDefaults = true, preferredSourceId = null } = {}) {
  ...
  const sourceSelected = loadSources && !state.editingDocumentId
    ? await loadSourceDocuments({ preferredSourceId })
    : false;
  updateDocumentPreview();
...
function resetDocumentFormForCreate() {
  $('#document-form').reset();
  $('#doc-recalculate-due-field').classList.add('hidden');
  $$('[data-doc-type-card]').forEach((card) => {
    card.disabled = false;
    card.classList.remove('unavailable');
  });
...
async function openDocumentModal(documentId = null, preferredType = null, preferredSourceId = null, metadataOnly = false) {
  const modalOpenSequence = ++state.documentModalOpenSequence;
  ...
  // Quick customer creation is optional. Never leave its expanded state
  // hanging over the next create/edit session.
  ...
  // เปิดหน้าต่างก่อนเสมอ: การโหลดลูกค้าต้องไม่ทำให้ผู้ใช้รู้สึกว่า
  // ปุ่มสร้างเอกสารกดไม่ติดหรือ Modal ค้างรอเครือข่าย.
  $('#document-modal').classList.remove('hidden');
  document.body.style.overflow = 'hidden';
  refreshIcons();
  const customerLoad = refreshDocumentCustomers();
  // Keep document history suggestions fresh without delaying the modal.
  // This also picks up items entered from another browser/session.
  refreshProductSuggestions().catch(() => undefined);

  if (state.editingDocumentId) {
    await customerLoad;
    if (modalOpenSequence !== state.documentModalOpenSequence) return;
    ...
    if (state.editingMetadataOnly) {
      $('#document-modal-subtitle').textContent = 'แก้ไขวันที่เอกสาร วันครบกำหนด เงื่อนไข หมายเหตุ และลายเซ็น โดยไม่เปลี่ยนเลขเอกสารหรือยอดเงิน';
      ...
      $('#doc-recalculate-due-field').classList.toggle('hidden', !doc.due_date);
      ['#doc-date','#doc-due-date','#doc-term-days','#doc-recalculate-due','#doc-remarks','#doc-payment-terms','#doc-validity-days','#doc-show-signature','#save-document','#wizard-back','#wizard-next'].forEach((selector) => { const element = $(selector); if (element) element.disabled = false; });
    }
    setWizardStep(2);
  } else {
    ...
    updateAllowedDocumentTypes();
    const preferredSourceSelected = await selectDocumentType(preferredType || 'QT', {
      loadSources: true,
      applyDefaults: true,
      preferredSourceId
    });
    if (preferredSourceId) {
      setWizardStep(2);
      if (!preferredSourceSelected) {
        showDocumentFormError('ไม่สามารถเลือกเอกสารต้นทางใบนี้ได้ กรุณากด “โหลดใหม่” หรือตรวจว่าสถานะเอกสารไม่ใช่ ยกเลิก ปฏิเสธ หรือชำระแล้ว');
      }
    } else {
      setWizardStep(1);
    }
...
function addDocumentLine(lineType = 'item', data = {}) {
  ...
  if (lineType === 'item') {
    row.innerHTML = `
      <div class="line-kind"><div class="line-order-controls"><label class="line-position-label"><span>ลำดับ</span><input class="line-position" type="number" min="1" step="1" inputmode="numeric" aria-label="ย้ายไปลำดับที่"></label><button class="move-line-up" type="button" aria-label="เลื่อนรายการขึ้น" title="เลื่อนขึ้น"><i data-lucide="arrow-up"></i></button><button class="move-line-down" type="button" aria-label="เลื่อนรายการลง" title="เลื่อนลง"><i data-lucide="arrow-down"></i></button><button class="drag-handle" type="button" aria-label="กดค้างแล้วลากเพื่อเรียงรายการ" title="กดค้างแล้วลากเพื่อเรียงรายการ"><i data-lucide="grip-vertical"></i></button></div><select class="line-item-type" aria-label="ประเภทรายการ"><option value="service">ค่าแรง</option><option value="product">สินค้า</option><option value="travel">เดินทาง</option><option value="other">อื่น ๆ</option></select></div>
    ...
    row.innerHTML = `
      <div class="line-kind"><div class="line-order-controls"><label class="line-position-label"><span>ลำดับ</span><input class="line-position" type="number" min="1" step="1" inputmode="numeric" aria-label="ย้ายไปลำดับที่"></label><button class="move-line-up" type="button" aria-label="เลื่อนรายการขึ้น" title="เลื่อนขึ้น"><i data-lucide="arrow-up"></i></button><button class="move-line-down" type="button" aria-label="เลื่อนรายการลง" title="เลื่อนลง"><i data-lucide="arrow-down"></i></button><button class="drag-handle" type="button" aria-label="กดค้างแล้วลากเพื่อเรียงรายการ" title="กดค้างแล้วลากเพื่อเรียงรายการ"><i data-lucide="grip-vertical"></i></button></div><select class="line-type"><option value="${lineType}">${lineType === 'section' ? 'หัวข้อ' : 'หมายเหตุ'}</option><option value="item">คิดเงิน</option><option value="${lineType === 'section' ? 'note' : 'section'}">${lineType === 'section' ? 'หมายเหตุ' : 'หัวข้อ'}</option></select></div>
  ...
  $('.remove-line', row).addEventListener('click', () => {
    ...
    refreshDocumentLineOrderControls();
    updateDocumentPreview();
  });
  ...
  if (lineType === 'item') {
    const descriptionInput = $('.line-description', row);
    const applySuggestedProduct = () => {
      const typedName = descriptionInput.value.trim().toLocaleLowerCase();
      const suggestions = state.productSuggestions.length ? state.productSuggestions : state.products;
      const product = suggestions.find((item) => (
        item.active !== false && String(item.name || '').trim().toLocaleLowerCase() === typedName
      ));

      // Do not leave a product ID attached when the user turns a suggested
      // item into a new line.  This is what allows the backend to save it as
      // a new catalog item after the document is saved.
      if (!product) {
    ...
    // `input` supports selecting an option with mouse, keyboard, and mobile
    // browsers; `change` remains as a fallback for native datalist behavior.
    descriptionInput.addEventListener('input', applySuggestedProduct);
    descriptionInput.addEventListener('change', applySuggestedProduct);
  }
  $('.move-line-up', row).addEventListener('click', () => moveDocumentLine(row, getDocumentLines().indexOf(row) - 1));
  $('.move-line-down', row).addEventListener('click', () => moveDocumentLine(row, getDocumentLines().indexOf(row) + 1));
  const positionInput = $('.line-position', row);
  const applyTypedPosition = () => {
    const value = positionInput.value.trim();
    if (!value) return refreshDocumentLineOrderControls();
    return moveDocumentLine(row, Number(value) - 1);
  };
  positionInput.addEventListener('change', applyTypedPosition);
  ...
  const dragHandle = $('.drag-handle', row);
  const armDrag = (event) => {
    if (event.button !== 0 || dragHandle.disabled) return;
    // Chrome starts HTML5 drag from the element that receives the pointer.
    // The old implementation made the button draggable, while the code moved
    // its parent row; that combination is unreliable.  Arm the row only for
    // this press so typing in every other field can never move it by mistake.
    activeDocumentLineDrag = row;
  ...
  row.addEventListener('dragstart', (event) => {
    if (row.dataset.dragArmed !== 'true' || dragHandle.disabled) {
      event.preventDefault();
      return;
    }
    activeDocumentLineDragOrder = getDocumentLines();
    event.dataTransfer.effectAllowed = 'move';
    event.dataTransfer.setData('text/plain', 'document-line');
    row.classList.add('dragging');
  });
  row.addEventListener('dragend', () => {
    if (activeDocumentLineDragOrder && !sameDocumentLineOrder(activeDocumentLineDragOrder, getDocumentLines())) {
      setDocumentItemOrderUndo(activeDocumentLineDragOrder);
      showToast('เรียงลำดับแล้ว กดย้อนกลับได้หากต้องการ');
    }
    activeDocumentLineDragOrder = null;
    resetDocumentLineDrag(row);
    refreshDocumentLineOrderControls();
    updateDocumentPreview();
  });
  $('#document-items').appendChild(row);
  configureDocumentLineDrag(row);
  refreshDocumentLineOrderControls();
  updateDocumentPreview();
  refreshIcons();
}

let activeDocumentLineDrag = null;
let activeDocumentLineDragOrder = null;
let documentItemOrderUndo = null;

function getDocumentLines() {
  return $$('.document-line', $('#document-items'));
}

function sameDocumentLineOrder(left, right) {
  return left.length === right.length && left.every((row, index) => row === right[index]);
}

function setDocumentItemOrderUndo(lines) {
  documentItemOrderUndo = [...lines];
  $('#undo-item-order')?.classList.remove('hidden');
}

function clearDocumentItemOrderUndo() {
  documentItemOrderUndo = null;
  $('#undo-item-order')?.classList.add('hidden');
}

function refreshDocumentLineOrderControls() {
  const lines = getDocumentLines();
  lines.forEach((row, index) => {
    const position = $('.line-position', row);
    if (position && document.activeElement !== position) position.value = String(index + 1);
    if (position) {
      position.max = String(lines.length);
      position.disabled = state.editingMetadataOnly;
    }
    const up = $('.move-line-up', row);
    const down = $('.move-line-down', row);
    if (up) up.disabled = state.editingMetadataOnly || index === 0;
    if (down) down.disabled = state.editingMetadataOnly || index === lines.length - 1;
    configureDocumentLineDrag(row);
  });
}

function moveDocumentLine(row, requestedIndex, { notify = true } = {}) {
  const lines = getDocumentLines();
  const currentIndex = lines.indexOf(row);
  const numericIndex = Number(requestedIndex);
  if (currentIndex < 0 || !Number.isFinite(numericIndex) || state.editingMetadataOnly) {
    refreshDocumentLineOrderControls();
    return false;
  }
  const targetIndex = Math.min(Math.max(Math.trunc(numericIndex), 0), lines.length - 1);
  if (targetIndex === currentIndex) {
    refreshDocumentLineOrderControls();
    return false;
  }
  setDocumentItemOrderUndo(lines);
  const remaining = lines.filter((line) => line !== row);
  $('#document-items').insertBefore(row, remaining[targetIndex] || null);
  refreshDocumentLineOrderControls();
  updateDocumentPreview();
  if (notify) showToast(`ย้ายรายการไปลำดับ ${targetIndex + 1} แล้ว`);
  return true;
}

function undoDocumentItemOrder() {
  if (!documentItemOrderUndo) return;
  const lines = documentItemOrderUndo.filter((row) => row.isConnected);
  if (!lines.length) return clearDocumentItemOrderUndo();
  lines.forEach((row) => $('#document-items').appendChild(row));
  clearDocumentItemOrderUndo();
  refreshDocumentLineOrderControls();
  updateDocumentPreview();
  showToast('ย้อนกลับลำดับรายการแล้ว');
}

function resetDocumentLineDrag(row = activeDocumentLineDrag) {
  if (!row) return;
  row.dataset.dragArmed = 'false';
  row.draggable = false;
  row.classList.remove('dragging');
  if (activeDocumentLineDrag === row) activeDocumentLineDrag = null;
}
...
function configureDocumentLineDrag(row) {
  const handle = $('.drag-handle', row);
  if (!handle) return;
  const disabled = state.editingMetadataOnly;
  // A line becomes draggable only while its 6-dot handle is being held.
  // Keeping the row locked at every other time prevents accidental movement.
  resetDocumentLineDrag(row);
  handle.draggable = false;
  handle.disabled = disabled;
  handle.setAttribute('aria-disabled', String(disabled));
  handle.title = disabled ? 'โหมดนี้แก้ไขได้เฉพาะข้อมูลเอกสาร' : 'กดค้างแล้วลากเพื่อเรียงรายการ';
...
// A row is draggable only while its handle is held. No form input can start a
// drag accidentally, and the row returns to its safe state on release.
document.addEventListener('pointerup', () => resetDocumentLineDrag());
document.addEventListener('pointercancel', () => resetDocumentLineDrag());
window.addEventListener('blur', () => resetDocumentLineDrag());

$('#undo-item-order').addEventListener('click', undoDocumentItemOrder);
$('#add-item').addEventListener('click', () => { clearDocumentItemOrderUndo(); addDocumentLine('item'); });
$('#add-section').addEventListener('click', () => { clearDocumentItemOrderUndo(); addDocumentLine('section'); });
$('#add-note').addEventListener('click', () => { clearDocumentItemOrderUndo(); addDocumentLine('note'); });
...
$('#doc-refresh-customers').addEventListener('click', () => refreshDocumentCustomers().then((loaded) => {
  if (loaded) showToast('อัปเดตรายชื่อลูกค้าแล้ว');
}));
$('#doc-retry-customers').addEventListener('click', () => refreshDocumentCustomers());
...
function sourceSelectionCopy(type) {
  ...
  if (type === 'IN') return ['เลือกใบเสนอราคาหรือใบส่งมอบงาน', 'ระบบคัดลอกรายการ ลูกค้า จำนวน และยอดมาให้อัตโนมัติ'];
  return ['เลือกเอกสารต้นทาง', 'ระบบจะเชื่อมสถานะและข้อมูลให้อัตโนมัติ'];
}

function sourceEmptyMessage(targetType) {
  if (targetType === 'BN') return 'ไม่พบใบแจ้งหนี้ที่พร้อมนำมาวางบิล หรือใบแจ้งหนี้ถูกวางบิลแล้ว';
  if (targetType === 'RC') return 'ไม่พบเอกสารที่พร้อมรับชำระ หรือเอกสารถูกออกใบเสร็จแล้ว';
  if (targetType === 'DO') return 'ไม่พบใบเสนอราคาที่พร้อมนำมาสร้างใบส่งมอบ';
  if (targetType === 'IN') return 'ไม่พบใบเสนอราคาหรือใบส่งมอบงานที่พร้อมนำมาแจ้งหนี้ คุณยังเพิ่มรายการเองเพื่อสร้างใบแจ้งหนี้โดยตรงได้';
  return 'ยังไม่มีเอกสารต้นทางที่พร้อมใช้งาน';
}

async function loadSourceDocuments({ preferredSourceId = null } = {}) {
  if (state.editingDocumentId) {
    $('#source-documents-box').classList.add('hidden');
    return false;
  }
  const requestSequence = ++state.sourceRequestSequence;
  updateAllowedDocumentTypes();
  ...
  box.classList.remove('hidden');
  list.innerHTML = '<div class="source-loading"><i data-lucide="loader-circle"></i><span>กำลังโหลดเอกสารต้นทางล่าสุด...</span></div>';
  refreshIcons();
  const result = await request(`/documents/sources?target_type=${targetType}&customer_id=${customerId}`, { cache: 'no-store' });
  if (requestSequence !== state.sourceRequestSequence) return false;
  const rows = Array.isArray(result.data) ? result.data : [];
  list.innerHTML = '';

  if (!rows.length) {
    $('#source-empty-message').textContent = sourceEmptyMessage(targetType);
  ...
  list.innerHTML = rows.map((documentRow) => `
    <label class="source-option source-document-card" for="source-document-${documentRow.id}">
      <input id="source-document-${documentRow.id}" type="${inputType}" name="source-document" value="${documentRow.id}" data-total="${Number(documentRow.grand_total || 0)}">
  ...
  const preferredInput = preferredSourceId
    ? $(`#source-documents-list input[value="${Number(preferredSourceId)}"]`)
    : null;
...
$('#source-documents-refresh').addEventListener('click', () => {
  ...
  try {
    ...
    const result = metadataOnly
      ? await request(`/documents/${editingId}/metadata`, { method:'PATCH', body:JSON.stringify({
          ...
          recalculate_due_date: $('#doc-recalculate-due').checked
        }) })
    ...
    // A newly typed line is saved to the catalog by the backend. Refresh the
    // local suggestion cache now so the next document can use it immediately
    // without requiring a full page reload or opening the products screen.
    const refreshSuggestions = editingId ? Promise.resolve() : refreshProductSuggestions().catch(() => undefined);
    await Promise.all([loadDashboard(), loadDocuments(), refreshSuggestions]);

...
router.get('/', authorize('customer.view'), validate(masterDataListSchema, 'query'), asyncHandler(async (req, res) => {
    ...
    const statusCondition = `(
        $3 = 'all'
        OR ($3 = 'active' AND active IS DISTINCT FROM FALSE)
...
// Dedicated, unpaginated source for document forms. Keeping this query separate
// prevents a list filter, page state, or legacy NULL active value from making
// the customer dropdown appear empty while the customer-management page works.
router.get('/document-options', authorize('customer.view'), asyncHandler(async (_req, res) => {
    res.set({
        'Cache-Control': 'no-store, no-cache, must-revalidate, private',
        Pragma: 'no-cache',
        Expires: '0'
...
router.put('/:id', authorize('customer.update'), validate(idSchema, 'params'), validate(customerSchema), asyncHandler(async (req, res) => {
    const updatedCustomer = await withTransaction(async (client) => {
        ...
        // Legacy customer rows can have active = NULL. They predate the
        // lifecycle flag and must remain usable until explicitly deactivated.
    ...
    const deactivatedCustomer = await withTransaction(async (client) => {
        const result = await client.query(
            `UPDATE customers SET
             ...
             WHERE id = $1 AND active IS DISTINCT FROM FALSE
             RETURNING *`,

...
/**
 * Store genuinely new billable lines for future document suggestions.
 * Existing catalog rows are intentionally left unchanged: choosing an old
 * item should keep its saved defaults, while a one-off edited line must not
 * silently overwrite the standard price or unit.
 */
async function saveNewCatalogItems(client, items, userId) {
    const candidates = new Map();
    for (const item of items) {
        if (item.line_type !== 'item' || item.product_id) continue;
        ...
        if (!candidates.has(key)) {
            candidates.set(key, {
                ...
                unit: String(item.unit || 'งาน').trim() || 'งาน',
    ...
    for (const candidate of candidates.values()) {
        // Serialise concurrent saves for the same normalized name. Products
        // has no name uniqueness constraint in older installations, so this
        // preserves the promise that auto-save never creates name duplicates.
        await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [candidate.name.toLocaleLowerCase()]);
        const existing = await client.query(
            'SELECT id FROM products WHERE LOWER(BTRIM(name)) = LOWER(BTRIM($1)) LIMIT 1 FOR KEY SHARE',
            [candidate.name]
        );
        ...
        const result = await client.query(
            `INSERT INTO products (sku, name, item_type, unit, price, category, active)
             VALUES (NULL, $1, $2, $3, $4, NULL, TRUE)
             RETURNING *`,
            [candidate.name, candidate.itemType, candidate.unit, candidate.price]
        );
...
async function createDocument({ body, userId }) {
    const documentId = await withTransaction(async (client) => {
        ...
        // Older customer records may have active = NULL.  Treat them as active
        // until someone explicitly deactivates them (active = FALSE).
        ...
        const autoSavedProducts = await saveNewCatalogItems(client, totals.items, userId);
        ...
        await writeAudit(client, {
            ...
            details: {
                ...
                sourceDocumentIds,
                autoSavedProductIds: autoSavedProducts.map((product) => product.id)
            }
        });
...
async function updateDocument({ id, body, userId, role }) {
    await withTransaction(async (client) => {
        ...
        // Keep the legacy-customer rule consistent with document creation.
        // A NULL lifecycle flag is usable; only FALSE is inactive.
...
async function updateDocumentMetadata({ id, body, userId, role }) {
    await withTransaction(async (client) => {
        await assertDocumentSchemaReady(client);
        const current = await getLockedDocument(client, id);
        if (!['admin', 'staff'].includes(role)) {
            throw new AppError(403, 'ไม่มีสิทธิ์แก้ไขข้อมูลเอกสาร', 'DOCUMENT_EDIT_NOT_ALLOWED');
        }
        if (current.status === 'CANCELLED') {
            throw new AppError(403, 'ไม่สามารถแก้ไขเอกสารที่ยกเลิกแล้ว', 'DOCUMENT_EDIT_NOT_ALLOWED');
        }
        if (current.status === 'PAID' && role !== 'admin') {
            throw new AppError(403, 'เอกสารที่รับชำระแล้วแก้ไขได้เฉพาะผู้ดูแลระบบ', 'ADMIN_REQUIRED');
        }

        let dueDate = body.due_date;
        if (body.recalculate_due_date && current.due_date) {
            const oldDate = new Date(`${current.document_date}T00:00:00Z`);
            const oldDue = new Date(`${current.due_date}T00:00:00Z`);
            const days = Math.max(Math.round((oldDue - oldDate) / 86400000), 0);
            const nextDue = new Date(`${body.document_date}T00:00:00Z`);
            nextDue.setUTCDate(nextDue.getUTCDate() + days);
            dueDate = nextDue.toISOString().slice(0, 10);
        }
        if (dueDate && dueDate < body.document_date) {
            throw new AppError(400, 'วันครบกำหนดต้องไม่ก่อนวันที่เอกสาร', 'INVALID_DUE_DATE');
        }
        ...
        await writeAudit(client, {
            ...
            details: {
                ...
                before: {
                    ...
                    quotationValidityDays: current.quotation_validity_days,
                    showSignature: current.show_signature
                },
                after: {
                    documentDate: body.document_date,
                    dueDate,
                    ...
                    quotationValidityDays: body.quotation_validity_days ?? null,
                    showSignature: Boolean(body.show_signature)
                }
            }
        });
    }, { name: 'document.update_metadata' });
    return getDocumentById(id);
}

async function getDocumentImpact(id) {
    const documentResult = await pool.query(
        `SELECT id, document_number, document_type, status, deleted_at
    ...
    const relationsResult = await pool.query(
        `SELECT r.id, r.relation_type,
                r.source_document_id AS source_id,
                r.target_document_id AS target_id,
    ...
    return { document, relations: relationsResult.rows };
}
